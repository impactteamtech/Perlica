      ...
      <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 w-full">
        {icons.map(({ label, icon: Icon }, i) => (
          <li key={i} className="flex justify-center">
            <button
              aria-label={label}
              className="flex flex-col items-center gap-3 p-6 rounded-xl bg-white shadow-md hover:shadow-xl hover:scale-110 hover:bg-blue-50 transition-transform duration-300 ease-out cursor-pointer w-full"
            >
